...
static int parent_codec_round_trip_valid(void) {
...
static struct dh_canary_admission_proof parent_proof(void) {
    return (struct dh_canary_admission_proof){
        .control_role = dh_parent_contract_fd_role(DH_CANARY_FD_CONTROL),
        .input_role = dh_parent_contract_fd_role(DH_CANARY_FD_INPUT),
        .mock_sink_role = dh_parent_contract_fd_role(DH_CANARY_FD_MOCK_SINK),
        .status_role = dh_parent_contract_fd_role(DH_CANARY_FD_STATUS),
        .ready_transition_valid = dh_parent_contract_accept_transition(
            0u, DH_CANARY_FRAME_READY
        ),
        .codec_round_trip_valid = parent_codec_round_trip_valid(),
    };
}
...
int dh_parent_entrypoint_admit(int argc, const char *const argv[]) {
    const struct dh_canary_admission_proof proof = parent_proof();
    return dh_parent_entrypoint_admit_from_proof(argc, argv, &proof);
}

#ifndef DH_CANARY_ENTRYPOINT_UNIT_TEST
int main(int argc, char *argv[]) {
    return dh_parent_entrypoint_admit(argc, (const char *const *)argv) == DH_CANARY_ADMISSION_ACCEPTED
               ? 0
               : 64;
}
#endif
